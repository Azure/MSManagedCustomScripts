...
[CmdletBinding()]
param (
    [Parameter(Mandatory = $true, HelpMessage = "Resource ID of the Azure MySQL Flexible Server")]
...
try {
    ...
    Write-Log "Step 3: Parsing MySQL server resource ID..." "INFO"
    ...
    Write-Log "Step 4: Executing MySQL server failover..." "INFO"
    $operationSuccess = Invoke-MySQLServerFailover `
        -ResourceGroupName $resourceInfo.ResourceGroup `
        -ServerName $resourceInfo.ServerName `
        -SubscriptionId $resourceInfo.SubscriptionId
    
    if ($operationSuccess) {
        Write-Log "MySQL server failover completed successfully" "SUCCESS"
    }
    else {
        throw "MySQL server failover operation failed"
    }
}
catch {
...
if ($operationSuccess) {
    Write-Log "MySQL Flexible Server failover operation completed successfully" "SUCCESS"
    Write-Log "The server should now be available after the failover process" "INFO"
}
else {
    Write-Log "MySQL Flexible Server failover operation failed" "ERROR"
    ...
    throw "Failover operation failed for MySQL server: $ResourceId"
}
